...
public class CommunicatingViewStrategy extends DefaultViewStrategy {
	...
	public void drawHelperGraphics(final Agent agent, final Graphics g, final int squareSize) {
		if (agent.getStrategy() instanceof CommunicatingAgentStrategy && agent.getStrategy().getState().equals(((CommunicatingAgentStrategy)agent.getStrategy()).RANDOMCOMMS)) {
			/*
			...
			if (agent.getStrategy() instanceof CommunicatingAgentStrategy) {
				...
				g.drawOval(toGraphicLoc(agent.getLoc().getX() - commDist, squareSize),
						toGraphicLoc(agent.getLoc().getY() - commDist, squareSize), toGraphicSize(commDist * 2, squareSize),
						toGraphicSize(commDist * 2, squareSize));
			}
		} else if (agent.getStrategy() instanceof CommunicatingAgentStrategy && agent.getStrategy().getState().equals(((CommunicatingAgentStrategy)agent.getStrategy()).GOTO)) {
			...
			agent.getStrategy().getTaskToDo().ifPresent((t) -> {
				...
				g.drawLine(toGraphicLoc(agent.getLoc().getX(), squareSize) + midPoint,
						toGraphicLoc(agent.getLoc().getY(), squareSize) + midPoint, toGraphicLoc(l.getX(), squareSize) + midPoint,
