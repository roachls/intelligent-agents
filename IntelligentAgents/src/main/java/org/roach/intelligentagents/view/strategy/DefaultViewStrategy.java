...
public class DefaultViewStrategy implements ViewStrategy {
	...
	public void draw(final Agent agent, final Graphics g, int squareSize) {
		...
		g.drawRect(toGraphicLoc(agent.getLoc().getX(), squareSize), toGraphicLoc(agent.getLoc().getY(), squareSize), squareSize, squareSize);
	}
	...
	@Override
	public void drawHelperGraphics(Agent agent, Graphics g, final int squareSize) {
